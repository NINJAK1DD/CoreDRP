...
# 0xFFFF is the boundary-test allocation in the conformance corpus rather than an application event.
v=json.loads((R/'docs/coredrp-v1-test-vectors.json').read_text())
if '0xFFFF' not in (R/'docs/coredrp-v1-test-vectors.json').read_text():raise SystemExit('boundary-test event type 0xFFFF missing from conformance corpus')
