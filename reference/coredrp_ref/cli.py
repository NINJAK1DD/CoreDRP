...
from .transport import tls,serve,drain
...
def main():
    ...
    for action in ('serve','sync'):
        ...
        else:q.add_argument('--wal',required=True,type=Path);q.add_argument('--receiver',required=True,type=identifier)
    ...
    if a.action=='bootstrap':bootstrap(os.environ['COREDRP_REF_DSN'],a.receiver,a.incarnation,a.sender,a.epoch,a.admin_id)
        ...
        with WAL(a.wal) as w:asyncio.run(drain(w,a.host,a.port,tls(a.ca,a.cert,a.key),a.receiver));print(json.dumps({'remembered_ack':w.state['ack']}))
    else:
