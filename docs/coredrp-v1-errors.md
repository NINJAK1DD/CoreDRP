# CoreDRP/1 Error Registry — Draft 0.4
...
| 10 | SPLIT_LOG | OPERATOR_INTERVENTION | Same epoch/sequence has different hash or receiver history cannot be verified | no | no |
...
| 15 | SEMANTIC_CONTRACT_MISMATCH | PERMANENT_CONFIGURATION | Selected profile/scope semantic digest mismatch | no | no |
| 16 | RECEIVER_DURABILITY_UNAVAILABLE | STREAM_RETRYABLE | Receiver cannot currently provide durable commit | reconnect | no |
| 17 | CLOCK_CONTRACT_VIOLATION | STREAM_RETRYABLE | Clock proof not presently trustworthy | yes/reconnect | no |
...
| 22 | RESOURCE_LIMIT_EXCEEDED | STREAM_RETRYABLE | Aggregate/window/batch resource bound exceeded and request can be split/retried | yes | no |
| 23 | CONTRACT_BINDING_CHANGED | OPERATOR_INTERVENTION | Epoch persisted capability/semantic binding changed | no | no |
...
| 29 | ADMIN_ACTION_CONFLICT | OPERATOR_INTERVENTION | optimistic state-version check failed | n/a | n/a |
| 30 | IDEMPOTENCY_KEY_CONFLICT | PERMANENT_CONFIGURATION | idempotency key reused for different canonical request/admission identity | n/a | n/a |
| 31 | INVALID_STATE_TRANSITION | OPERATOR_INTERVENTION | profile/admin state-machine transition invalid | no | profile-dependent |
| 32 | INVALID_EVENT_PLACEMENT | PERMANENT_CONFIGURATION | Known event type used on a forbidden lane or scope form | no | no |
| 33 | ATOMIC_RESOURCE_LIMIT_EXCEEDED | PERMANENT_CONFIGURATION | Intrinsic single-object limit exceeded; splitting/retry cannot make it valid | no | no |
| 34 | RECEIVER_ID_CHANGED | OPERATOR_INTERVENTION | Authenticated/pinned logical receiver identity changed unexpectedly | no | no |

`ProtocolError.disposition` is redundant transport metadata. The table above is authoritative. A received code/disposition mismatch is `MALFORMED_FRAME`.

`RESOURCE_LIMIT_EXCEEDED` is reserved for retryable aggregate conditions such as a batch/window that can be split or reduced. Intrinsic violations such as an overlong scope use `ATOMIC_RESOURCE_LIMIT_EXCEEDED` (or a more specific permanent code such as `EVENT_TOO_LARGE`).
