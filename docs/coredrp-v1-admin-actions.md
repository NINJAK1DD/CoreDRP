...
- effective time MUST be beyond `PayoutSafeThrough(scope) + applicable_clock_uncertainty` using checked arithmetic.
...
Membership corrections require field 6 sender UUID. Mode corrections require field 6 zero-length/absent semantic value as specified by the policy registry.
...
## 6. Settlement override

`SETTLE_WITHOUT_FENCE_OVERRIDE` applies to exactly one settlement identifier. The durable result records the settlement, scope, reason, current gap/quarantine/policy state and operator audit identity. It MUST NOT be interpreted as a policy correction, gap reconciliation, or reusable safety proof.
