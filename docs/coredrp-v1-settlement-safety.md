...
## 9. Retained settlement audit evidence

`SettlementEvidenceSummaryV1` and its component hashes are necessary but are not by themselves sufficient to delete original financial payloads. For PPLNS/PPLNSBF/PROP, Profile 1.1 chooses conservative full-payload retention: the complete accepted `0x0200` source payloads and Core identity/order headers for the settlement evidence interval MUST remain durably available after pruning ordinary database rows. This includes every eligible projection in the interval, not just shares that received a nonzero amount, plus the winning accounting event when a finder reserve is paid outside the window. Retain the immediately preceding eligible share (if any) for a filled window's boundary proof, or the complete history to the scope origin for an underfilled window. Required checkpoint/membership/contract proofs must establish interval completeness; a bag of selected rows alone is not such a proof.

Before any ordinary evidence deletion, atomically persist `SettlementAuditBundleV1` and its SHA-256 in the immutable final settlement record, together with the summary SHA-256. The exact schemas below are also in `coredrp-v1-request-schemas.json`; their record/length/presence primitives are those of the request registry.

`SettlementAuditBundleV1`: version 1; `summary_hash:hash32`; `inputs:SettlementInputV1`; `events:array AuditEventV1`.

`SettlementInputV1` in order: `block_hash:hash32`, `block_height:u64`, `gross_reward:ascii`, `deductions:array DeductionV1`, `distributable_reward:ascii`, `finder_accounting_id:?uuid16`, `finder_percentage:ascii`.

`DeductionV1`: version 1; `destination:bytes` (exact payout identity bytes); `amount:ascii`. Destinations are nonempty and unique by decoded bytes; sort complete records lexicographically. Monetary inputs are nonnegative canonical Decimal38Scale24; require `gross_reward = sum(deductions) + distributable_reward` exactly. These are actual immutable deductions and reward inputs from the final transaction, never recomputed from current configuration. Finder percentage must equal the bound PPLNSBF policy and be zero/absent finder for other schemes; the finder UUID must satisfy settlement-scheme §8's winning projection linkage. Retain proof of the accepted block reward and deduction authorizations; a locally invented reward record does not establish correctness.

`AuditEventV1`: version 1; `event_time:i64`, `sender:uuid16`, `sequence:u64`, `relay:uuid16`, `epoch:uuid16`, `lane:u8`, `event_type:u16`, `scope:bytes`, `previous_chain_hash:hash32`, `chain_hash:hash32`, `payload:bytes`. Sort by the complete canonical Core tuple `(event_time,sender,sequence,relay)` with duplicate tuples rejected. Exact original payload bytes are retained, never a JSON/protobuf reserialization. Recompute the Core payload hash and event-chain link from each retained record and compare to retained accepted-chain anchors. A header/hash with no original payload is insufficient.

The auditor MUST parse each retained accounting payload under its accepting contracts; verify generated created time and projection scopes; reconstruct assigned and network difficulty bits, bound adjustment, quotient, full tuple and exact window contribution/fraction; validate finder linkage; recalculate allocation and dust from the retained reward/deductions; and match every participant UUID/destination/amount and effect identity, including zero allocations and same-miner aggregation. A changed difficulty, missing source event, altered reward/deduction, incorrect finder or cutoff must fail audit even if an attacker recomputes the summary hashes. Interval completeness and accepted-chain authentication are independent required inputs, not assumptions inferred from a matching digest.

The bundle and all its referenced payloads, canonical inputs, contract/policy source bytes, chain/completeness and authorization proofs MUST be retained for the lifetime of the final settlement audit record. `SettlementPruneSafe` permits deleting only redundant ordinary rows after this durable replacement; it MUST NOT delete these retained payloads. If complete reconstruction or any required proof is unavailable, retain the original ordinary evidence and return false. This is deliberately higher storage cost than a compact contribution proof. A future compact scheme requires a new audited representation; payload hashes cannot substitute for difficulty or reward inputs.
